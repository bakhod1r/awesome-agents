...
/plugin marketplace add anthropics/claude-plugins-community</code></pre>
  ...
  <h3>Cut the token bill</h3>
  <p class="lede">{len(AGENTS)} agents with a ten-section contract each is a lot of prose.
  <a href="https://github.com/juliusbrussee/caveman">caveman</a> strips filler from the model's
  prose while leaving code, commands, file paths, and error strings byte-identical — so agent
  output stays quotable and diffable, only the padding goes.</p>
<pre><code>/plugin marketplace add juliusbrussee/caveman
/plugin install caveman@caveman
/caveman ultra                                    # or lite / full</code></pre>
  <p class="lede">It compresses <b>output</b>, not reasoning: an agent still reads the same
  files and applies the same Quality Bar. Measure your own saving with <code>/caveman-stats</code>
  rather than trusting a headline number.</p>
